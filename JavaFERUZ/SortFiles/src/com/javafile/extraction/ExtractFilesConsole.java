...
public class ExtractFilesConsole extends JPanel implements ActionListener {
...
//           }
			 ...
			 FindJavaFiles f1 = new FindJavaFiles();
             String sourcePath = DESTINATION_DIRECTORY;

             File destinationFolder = FileSystemView.getFileSystemView().getHomeDirectory();
             destinationFolder = destinationFolder.getAbsoluteFile();
             destinationFolder.mkdirs();
             String destinationPath = destinationFolder+"\\StudentFiles\\";
             File file = new File(sourcePath);

             app = new Application(f1);
             app.sortTheMess(file, DESTINATION_DIRECTORY);
             app.display(DESTINATION_DIRECTORY);

             f1.getJavaFiles(file);
             try {
                 f1.createDestinationStudentsFolder(sourcePath, destinationPath);
             } catch (IOException e1) {
                 e1.printStackTrace();
             }
             f1.constructSourcePath(file);
             f1.constructDestinationPath();
             f1.sourceDestinationMapping();
//			 
	 ...
	 public static void main(String[] args) {
		 JFrame frame = new JFrame("FILE PICKER");
			...
			ExtractFilesConsole myPanel = new ExtractFilesConsole();
